...
// ---- Fase 3 (revisada): comparativo financeiro + gráficos, sem texto longo ----

function BarPair({ labelA, valueA, labelB, valueB, colorA = "var(--ink-soft)", colorB = "var(--teal-deep)" }) {
  ...
  return (
    <svg viewBox="0 0 220 150" className="chart-svg" role="img" aria-label="Projeção de economia acumulada em 3 anos">
      ...
      {anos.map((ano, i) => {
        ...
        return (
          <g key={ano}>
            <rect x={x} y={h + 20 - barH(v)} width={barW} height={barH(v)} rx="2" style={{ fill: "var(--gold-deep)" }} />
...
export function ReportPanel({ documento, financeiro, cliente, onGerar, gerando, gerarError, podeGerar, onExport }) {
  ...
  const economiaTotalAnual = estrategias.reduce((s, e) => s + (e.economiaAnualEstimada || 0), 0);
  const impostosAtuais = Number(financeiro.impostosAtuaisAnual) || 0;
  const faturamento = Number(financeiro.faturamentoAnual) || 0;
  const impostosProjetados = Math.max(0, impostosAtuais - economiaTotalAnual);
  const cargaAtual = faturamento ? (impostosAtuais / faturamento) * 100 : 0;
  const cargaProjetada = faturamento ? (impostosProjetados / faturamento) * 100 : 0;

  return (
    <div className="doc-panel print-area">
      <div className="doc-header no-print-flex">
        <div>
          <p className="eyebrow">Relatório de estratégia tributária</p>
          <p className="doc-cliente">{cliente.razaoSocial}</p>
        </div>
        {temRelatorio && <button className="btn btn-teal no-print" onClick={onExport}>Exportar (PDF)</button>}
      </div>
      ...
      {!temRelatorio ? (
        <div className="no-print" style={{ textAlign: "center", padding: "10px 2px 6px" }}>
          <p className="empty-sub" style={{ marginBottom: 14 }}>
            Preencha faturamento e impostos atuais, responda o roteiro, e gere o relatório consolidado — uma única
            chamada de IA no final, não uma a cada resposta.
          </p>
        ...
        <>
          <div className="kpi-row">
            <div className="kpi-card">
              ...
              <span className="kpi-value">{formatBRL(faturamento)}</span>
            </div>
            <div className="kpi-card">
              <span className="kpi-label">Impostos hoje</span>
              <span className="kpi-value">{formatBRL(impostosAtuais)}</span>
          ...
          <div className="charts-row">
            <div>
              ...
              <BarPair labelA="Hoje" valueA={impostosAtuais} labelB="Projetado" valueB={impostosProjetados} />
            </div>
            <div>
