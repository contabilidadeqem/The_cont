...
export function formatBRL2(v) {
  const n = Number(v);
  if (!Number.isFinite(n)) return "—";
  return n.toLocaleString("pt-BR", { style: "currency", currency: "BRL", minimumFractionDigits: 2, maximumFractionDigits: 2 });
}

// Formato curto usado na frase-âncora do relatório: "R$ 1.440 mil"
export function formatBRLMil(v) {
  const n = Number(v) || 0;
  return `R$ ${Math.round(n / 1000).toLocaleString("pt-BR")} mil`;
}

// Converte dígitos crus (centavos, ex: "144000000") em número em reais
export function centsToNumber(digits) {
  const d = (digits || "").replace(/\D/g, "");
  return d ? Number(d) / 100 : 0;
}

// Formata um número em reais como string de exibição com 2 casas (sem "R$")
