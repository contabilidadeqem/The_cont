...
import { QuestionCard, ReportPanel } from "@/components/ui";
...
export default function ReuniaoPage({ params }) {
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        await fetch(`/api/meetings/${meetingId}`, {
          ...
          body: JSON.stringify({ perguntas: next.perguntas, documento: next.documento, ...extra }),
        });
      } catch {
  ...
  const updateMeeting = useCallback((updater, extra) => {
    setMeeting((prev) => {
  ...
  const setFinanceiro = (campo, valor) => {
    updateMeeting(
      (prev) => ({ ...prev, perfilSnapshot: { ...prev.perfilSnapshot, [campo]: valor } }),
      { perfilSnapshot: { ...meetingRef.current.perfilSnapshot, [campo]: valor } }
    );
  };

  const podeGerarRelatorio =
    Number(meeting?.perfilSnapshot?.faturamentoAnual) > 0 &&
    Number(meeting?.perfilSnapshot?.impostosAtuaisAnual) >= 0 &&
  ...
  const gerarRelatorio = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/analise", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ perfil: atual.perfilSnapshot, perguntasRespondidas }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || "Falha ao gerar relatório.");
      const novoDocumento = { estrategias: json.estrategias, referenciaCurta: json.referenciaCurta, geradoEm: new Date().toISOString() };
    ...
    <div className="shell shell-wide">
      ...
      <section className="panel panel-profile no-print">
        ...
        <div className="field-grid">
          <Field2 label="Faturamento anual (R$)">
            <input
              className="input" type="number" min="0" placeholder="Ex: 1200000"
              defaultValue={meeting.perfilSnapshot.faturamentoAnual || ""}
              onBlur={(e) => setFinanceiro("faturamentoAnual", e.target.value)}
            />
          </Field2>
          <Field2 label="Impostos pagos por ano (R$)">
            <input
              className="input" type="number" min="0" placeholder="Ex: 240000"
              defaultValue={meeting.perfilSnapshot.impostosAtuaisAnual || ""}
              onBlur={(e) => setFinanceiro("impostosAtuaisAnual", e.target.value)}
            />
          </Field2>
        </div>
      </section>
